...
const SECOND_GATE_PIN = '616161'

const targetMap: Record<string, string> = {
  textile: '/textile-v2/index.html',
  logistics: '/la-vinda/platform/collection/logistics',
  fmcg: '/mafi-presentation/index.html',
  energy: '/la-vinda/platform/collection/energy',
}

function CollectionAccessContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const target = (searchParams.get('target') || '').toLowerCase()
  const destination = targetMap[target] || '/la-vinda/platform'
  ...
  const submitPin = async (pin: string) => {
    ...
    if (pin === SECOND_GATE_PIN) {
      setStatus('success')
      setTimeout(() => {
        if (destination.startsWith('/textile-v2/') || destination.startsWith('/mafi-presentation/')) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Enter the same six-digit code to open this collection.
        </p>
...
export default function CollectionAccessPage() {
  return (
    <Suspense fallback={null}>
      <CollectionAccessContent />
    </Suspense>
  )
}
